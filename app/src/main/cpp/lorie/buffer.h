...
#define AHARDWAREBUFFER_FORMAT_B8G8R8A8_UNORM                                  \
  5 // Stands to HAL_PIXEL_FORMAT_BGRA_8888
 ...
 * @return negative value on error, positive or zero  in the case of file
 * descriptor.
 */
 ...
 * It is a reference counted object. The returned buffer has a reference count
 * of 1.
 *
 * @param width width of buffer.
 * @param height height of buffer.
 * @param format format of buffer. Accepts AHARDWAREBUFFER_FORMAT_B8G8R8A8_UNORM
 * or AHARDWAREBUFFER_FORMAT_R8G8B8X8_UNORM
 * @param type type of buffer. Accepts LORIEBUFFER_REGULAR or
 * LORIEBUFFER_AHARDWAREBUFFER.
 * @return returns the buffer itself or NULL on failure.
 */
LorieBuffer *LorieBuffer_allocate(int32_t width, int32_t height, int8_t format,
                                  int8_t type);
...
STATIC_INLINE void LorieBuffer_acquire(LorieBuffer *buffer) {
  ...
  __sync_fetch_and_add((int *)buffer,
...
STATIC_INLINE void LorieBuffer_release(LorieBuffer *buffer) {
  ...
  if (buffer && __sync_fetch_and_sub((int *)buffer, 1) ==
                    1) // refcount is the first object in the struct
    __LorieBuffer_free(buffer);
}

/**
 * Return a description of the LorieBuffer in the passed LorieBuffer_Desc
 * struct.
 *
 ...
 */
int LorieBuffer_lock(LorieBuffer *buffer, AHardwareBuffer_Desc *outDesc,
                     void **out);
 ...
 * @param outBuffer
 */
void LorieBuffer_recvHandleFromUnixSocket(int socketFd,
                                          LorieBuffer **outBuffer);
void LorieBuffer_importFD(int fd, int32_t width, int32_t height, int32_t stride,
                          int8_t format, LorieBuffer **outBuffer);
int LorieBuffer_getFD(LorieBuffer *buffer);
